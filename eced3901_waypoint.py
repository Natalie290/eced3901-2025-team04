...
class NavigateSquare(Node):
    ...
    def run_servo(self):
        try:
            result = subprocess.run(
                ["ampy", "--port", "/dev/ttyUSB0", "run", "servo_script.py"], 
    ...
    def turn_complete90(self):
        """Check if a ±90-degree turn is complete"""
        if self.yaw is None:
            self.get_logger().error("Yaw value is None, cannot complete turn.")
            return False  # Prevent further execution if yaw is None

        # Ensure target yaw wraps correctly around 360
        target_yaw = (self.prev_yaw + self.turn_angle) % 360

        # Adjust target yaw to force shortest rotation path
        if target_yaw > 180:
            target_yaw -= 360  # Convert to negative equivalent for correct comparison

        if self.yaw > 180:
            yaw_adjusted = self.yaw - 360  # Convert yaw to same range
        else:
            yaw_adjusted = self.yaw

        # Calculate the shortest angle difference
        angle_diff = target_yaw - yaw_adjusted

        self.get_logger().info(f"Yaw: {self.yaw}, Target Yaw: {target_yaw}, Angle Diff: {angle_diff}")

        # Allow a small tolerance (e.g., 2 degrees) to avoid precision errors
        if abs(angle_diff) < 2:
            self.turning = False
            return True
